...
BOOLEAN G_PerformAsyncIo;		// 异步执行标志
BOOLEAN G_LimitedLoops;			// 异步执行次数是否有限的标志位
ULONG G_AsyncIoLoopsNum;		// 异步执行次数
...
*/
{
	...
	if (!WriteFile(hDevice,
		...
		printf("PerformWriteReadTest: WriteFile failed: Error %d\n", GetLastError());
